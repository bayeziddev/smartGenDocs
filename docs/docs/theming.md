...
SmartGen Docs has two independent layers of theming, and it's worth being clear about which one you mean before you start editing files:

1. **Structural themes** — `theme.name` in `smartgen.yml` (`default`, `education`, `book`, or a theme you add). This picks the whole page layout: the HTML structure, the navigation pattern, the CSS design tokens, the fonts, the motion. Switching this is switching the kind of site SmartGen Docs generates — a curriculum, a novel-style reading experience, a conventional docs site — not just its colors.
2. **Color modes** — the Light/Dark/Sepia/etc. switcher every structural theme ships with, driven by a `data-theme` attribute and remembered per-reader in `localStorage`. This is what most of this guide covers, because it's the layer most contributors touch.

Everything here is hand-authored CSS and vanilla JavaScript — no theming framework, no icon library, no third-party stylesheet or font CDN, in keeping with this project's zero-third-party-dependency goal.

## Structural themes

| Theme | `theme.name` | Layout |
|---|---|---|
| **Default** (aka "premium") | `default` or `premium` | Conventional docs layout: collapsible sidebar nav, search, breadcrumbs, Previous/Next footer. Four color modes (Light, Dark, Sepia, High Contrast). |
| **Education Docs** | `education` | Curriculum layout: a "Module → Lesson" sidebar with per-lesson visited/active dots, a top progress rail plus a progress ring in the header showing how far through the whole doc set the reader is, a card-framed lesson body with a client-computed read-time, and "Previous/Next Lesson" cards instead of plain links. Warm parchment + forest-green palette. Two color modes (Day, Night). |
| **Book / Writer Docs** | `book` | Focused reading layout: a narrow, serif, ~68-character measure; a drop cap on the opening paragraph; a collapsible "Contents" rail that can be hidden entirely with a **focused reading mode** toggle; a scroll-position reading-progress line; and page-turn-styled Previous/Next controls. Cool paper-white + ink-blue palette. Two color modes (Day, Night). |

Pick one with `theme.name` in `smartgen.yml`:

```yaml
theme:
  name: education   # or: default, book
```

If `theme.name` is missing, misspelled, or points at a theme directory with no content template, the build falls back to `default` and prints a warning listing the themes it actually found — a bad value here degrades gracefully instead of failing the build.

### How structural themes are resolved

This is the job of `smartgen_docs/theme_engine.py`'s `ThemeEngine` class, used by `core.py`'s `Builder`:

- Every theme lives in its own directory under `smartgen_docs/themes/<name>/`, with its own `base.html` (or `base_premium.html`, for `default`), `page.html`, and `static/` folder. Nothing about one theme's markup, CSS, or asset filenames can collide with another's.
- `themes/_shared/` is not a selectable theme — it's include-only. Any theme's templates can pull in the color-mode switcher, the pre-paint FOUC-prevention script, and the active-nav-link script from there with a plain `{% include %}`, instead of copy-pasting them. See `smartgen_docs/themes/_shared/README.md` for the full partial reference.
- Static assets: the `default` theme keeps its historical, un-namespaced `site/static/` output path (so nothing already linking to it breaks); every other theme's assets are copied to `site/static/<theme-name>/` instead, so two themes can both ship, say, `static/css/main.css` without clobbering each other.
- Every theme's `Environment` is a `ChoiceLoader` that checks the requested theme's own directory first, then `_shared/`, then `default/` — so a new theme can also `{% extends %}` or `{% include %}` anything from the default theme it doesn't want to reimplement.

### Adding a whole new structural theme

Unlike adding a color mode (below), a new structural theme is a genuinely different layout — there's no config toggle for it, you write templates and CSS. At minimum:

```
smartgen_docs/themes/<name>/
  base.html          # your layout; {% include "partials/..." %} from _shared for the switcher/prepaint/active-nav behavior
  page.html           # {% extends "base.html" %} + {% block content %}{{ content|safe }}{% endblock %}
  static/css/<name>.css   # your design tokens (typography, spacing, easing curves) + structural CSS
```

Nothing in `core.py` or `theme_engine.py` needs to change — `ThemeEngine.available_themes()` picks up any directory under `themes/` (other than `_shared`) that has a `page.html`. The context every page template renders with includes `title`, `content` (the converted Markdown, pass through the `|safe` filter), `config`, `nav` (the same nested structure as `smartgen.yml`'s `nav:` key), `current_page`, `breadcrumbs`, `prev_page` / `next_page`, `current_depth`, `url_for(type, filename)`, `theme_name`, and `current_index` / `total_pages` / `progress_percent` (generic "how far through the docs is this" data any theme can use for a progress indicator — themes that don't care simply ignore them). The `education` and `book` themes are complete, working examples of two very different layouts built from the same shared partials and the same context variables — read their `base.html` files alongside this guide.

## Color modes in the Default theme

The `default` theme ships four:
